...
export const getRecommendationByMood = async (req, res) => {
  ...
  try {
    const groqResponse = await groqProvider.generate({
      prompt: `You are a mental health assistant. Based on the user's mood, respond ONLY in JSON with 3 keys: "writingPrompt", "activity", and "message".
...
Mood: ${mood}`
    ...
    res.status(200).json({
      ...
      note: 'Fallback used due to Groq error'
    });
  }
};
